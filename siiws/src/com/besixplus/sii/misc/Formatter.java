		...
		if(myObject == null){
						...
						if(tmpMethods[j].getName().startsWith("get")){
							...
							try {
								if(tmpMethods[j].getReturnType() == byte.class)
									tmpMethod.addText( Base64.encodeBytes((byte[])tmpMethods[j].invoke(this.myObjects.get(i), "")) );
								else{
			...
			for(int j = 0; j < tmpMethods.length ; j++){
				if(tmpMethods[j].getName().startsWith("get")){
					...
					try {
						if(tmpMethods[j].getReturnType() == byte.class)
							tmpMethod.addText( Base64.encodeBytes((byte[])tmpMethods[j].invoke(this.myObject, "")) );
						else{
