...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    init : function(){
        ...
        var container = new Ext.ux.TaskBarContainer({
            el: 'ux-taskbar',
            layout: 'border',
            items: [sbBox,this.tbPanel,this.cklPanel]
        });
...
Ext.ux.ClockPanel = Ext.extend(Ext.BoxComponent, {
    initComponent : function(){
		...
		this.clockBtn = new Ext.Button({
			   ...
			   template: new Ext.Template(
						'<table border="0" cellpadding="0" cellspacing="0" class="x-btn"><tbody><tr>',
						'<td ><img id="user-info" src="resources/images/loading1.gif"/></td><td ><em unselectable="on"><button class="x-btn-text" type="{1}" style="height:30px;" id="ux-clockbutton-text">{0}</button></em></td><td ><i> </i></td>',
						'</tr></tbody></table>')
			});    
		
		//this.setWidth(this.clockBtn.getWidth());
