...
Ext.Desktop = function(app){
    ...
    var shortcuts = Ext.get('divShortcuts');
    ...
    if(shortcuts){
        shortcuts.on('click', function(e, t){
            if(t = e.getTarget('dt', shortcuts)){
                ...
                var module = null;
                if(t.id.indexOf('-shortcut')>=1){
                    module = app.getModule(t.id.replace('-shortcut', ''));
                    if(module!==null && module !== undefined && module.trim().length >0){
                        module.createWindow();
                    }else{
                        alert('El vinculo no esta enlazado a ningun modulo!.');
                    }
                }else if(t.id.indexOf('-subitem')>=1){
                    module = t.id.replace('-subitem', '');
                    try{
                        var win = eval("desktop.getWindow('win"+module+"')");
                        if (!win){
                            win = eval('new '+module+'(desktop);');
                        }
                        win.show();
                    }catch(inErr){
                        Ext.Msg.show({
                            title:APP_TITLE,
                            msg:'Se present\u00F3 un problema.\nError:'+inErr+'.\n'+ERR_MESSAGE,
                            buttons: Ext.Msg.OK,
