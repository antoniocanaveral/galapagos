...
Ext.ux.grid.CheckColumn.prototype ={
    ...
    * @param object
    */
    onMouseDown: function(eventObject, htmlElement, object) {
        if (Ext.fly(htmlElement).hasClass(this.createId())) {
            eventObject.stopEvent();
           // if (this.editable && this.editable === true) {
                var view = this.grid.getView();
                var row = view.findRowIndex(htmlElement);
                var col = view.findCellIndex(htmlElement);
                ...
                if (this.grid.fireEvent("beforedit", evtbefore) !== false && !evtbefore.cancel) {
                    var evtedit = {
                        grid: this.grid,
                        record: record,
                        field: field,
                    ...
                    if (this.grid.fireEvent("validateedit", evtedit) !== false && !evtedit.cancel) {
                        record.set(this.dataIndex, !record.data[this.dataIndex]);
                        this.grid.fireEvent("afteredit", evtedit);
                    }
                }
            //}
        }
