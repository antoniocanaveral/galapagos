...
UPDATE sii.cgg_res_tipo_solicitud_regla set crtt_codigo='CRTT5';
...
BEGIN
	...
	WHERE CRVAL_ESTADO AND
		...
		(CRTSE.CRTT_CODIGO IS NULL OR CRTSE.CRTT_CODIGO='' OR CRTSE.CRTT_CODIGO=IN_CRTT_CODIGO);
	RETURN NEXT TMP_REF;
END
$BODY$
...
CREATE OR REPLACE FUNCTION f_cgg_res_seguimiento_select1(in_user_name character varying, in_start_index integer, in_limit integer, in_sort_field_name character varying, in_direction character varying, in_find_text text, in_crtst_codigo character varying)
  RETURNS SETOF refcursor AS
$BODY$
DECLARE
	TMP_REF REFCURSOR;
...
BEGIN
	...
	FROM (
				...
				T.CRTRA_ORDEN,
				T.CRETT_CODIGO,
				SII.F_CGG_RES_PERSONA_SELECT_NOMBRES(T.REP_CRPER_CODIGO) AS REP_CRPER_CODIGO,
				(SELECT TST.CRTST_DESCRIPCION FROM SII.CGG_RES_TIPO_SOLICITUD_TRAMITE TST WHERE TST.CRTST_CODIGO = T.CHANGE_CRTST_CODIGO) CHANGE_CRTST_CODIGO,
				(select tt.crtt_nombre from cgg_tipo_tramite tt where TT.CRTT_CODIGO=T.CRTT_CODIGO) CRTT_CODIGO
