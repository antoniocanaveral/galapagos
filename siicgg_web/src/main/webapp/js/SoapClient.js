...
function SOAPClientParameters(){
    ...
    this.toXml = function()
    {
        ...
        {			
            if(p!=='$family' &&  p!=='anode' && typeof(this._data[p])!=="function"){
                switch(typeof(this._data[p])){
                    case "string":
                    case "number":
                    case "boolean":
                    case "object":
                        if(this._data[p] !== null && this._data[p] !== undefined){
                            xml += "<" + p + ">" + SOAPClientParameters._serialize(this._data[p]) + "</" + p + ">";
                        }else{
                            xml += '<' + p + ' xsi:nil="true"/>';
                        }
                        break;
		...
		if (typeof(this._data.sort) !== "function" ){
			var cad=/<sort>/;
			if (!cad.test(xml)){
			   if(this._data[p] !== null && this._data[p] !== undefined)
					xml += "<sort>" + SOAPClientParameters._serialize(this._data.sort) + "</sort>";
				else
					xml += '<sort xsi:nil="true"/>';						
			}
		}
        return xml;
    }
    this.get = function(inParamName){
        var outResult = null;
        for(var p in this._data){
            if(p == inParamName){
                outResult = this._data[p];
            }
        }
        return outResult;
    }
    ...
    if(async)
    {
        xmlHttp.onreadystatechange = function()
        {
            if(xmlHttp.readyState == 4){
                var valorResponse = xmlHttp.responseText.substring(0,38);
				if ( valorResponse == '<?xml version="1.0" encoding="UTF-8"?>')
...
SOAPClient._node2object = function(node, wsdlTypes)
{
    ...
    /*if (node.childNodes.length == 2 && (node.childNodes[0].nodeType == 3 || node.childNodes[0].nodeType == 4))
	{
		var
		return SOAPClient._node2object(node.childNodes[0], wsdlTypes);
	}*/
